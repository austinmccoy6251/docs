...
Now, suppose we want to be able to filter the search results by the `company_size` column. We would add `company_size` as a filter column and then reference it in the `filter_query` parameter of a search.

Our new dataframe now looks like this:

| company_description | company_name | id | company_size |
|-----------|-----|-----|-----|
| 'Acme is a startup that makes widgets' | 'Acme' | 1 | 'large' |
| 'Wayne Enterprises is a startup that makes widgets' | 'Wayne Enterprises' | 2 | 'large' |
| 'Parker Industries is a startup that makes widgets' | 'Parker Industries' | 3 | 'small' |

You can now reference `company_size` in the `filter_query` parameter of a search. For example, you could filter the search results to only include companies with the `company_size` of 'large' by setting the `filter_query` parameter to `company_size == 'large'`.
