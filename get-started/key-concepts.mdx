...
3. [Datasets](/datasets/understanding-datasets) are two-column Pandas dataframes saved as parquet files with two columns (`embedding` and `tags`), and only one at a time may be activated.
